class MusiciansController < ApplicationController
  ...
  def show
    breadcrumbs.add 'track', track_show_url
    ...
    @@spots_uris = spotify_tracks[1]                                           #spotify_uris for POST tracks to playlist
  ...
  def create
     if session[:uid] == nil                                                  #no user, no playlist
       ...
       playlist = Musician.create_playlist(session[:uid])                     #create empty playlist
       Musician.fill_playlist(@@spots_uris)                                   #fill empty playlist with tracks from musician show page
